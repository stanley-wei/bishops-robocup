...
def UpdateMap(occupancy_grid):
    ...
    if drive_mode != "stop":
        ...
        if drive_mode == "drive":
        ...
        elif drive_mode == "follower":
            follow_dist = 0.2
        
            poses_temp = past_follow_points.poses
            for i in range(len(poses_temp)):
                if (target_point.x - poses_temp[len(poses_temp) - 1].position.x)**2 + (target_point.y - poses_temp[len(poses_temp) - 1].position.y) ** 2 < follow_dist:
                    poses_temp = poses_temp[:-1]
                else:
                    break

            found_point = False
            for i in range(len(poses_temp)):
                new_target_point = poses_temp[len(poses_temp) - i - 1].position
                target_grid_position = PointToGridCoords(new_target_point)
                if processed_grid.data[target_grid_position[0], target_grid_position[1]] == 0:
                    found_point = True
                    break

            if found_point == False:
                x_dist = target_point.x - robot_position.x
                y_dist = target_point.y - robot_position.y
                vector_heading = math.atan2(y_dist, x_dist)
                new_x_dist = x_dist - follow_dist * math.cos(vector_heading)
                new_y_dist = y_dist - follow_dist * math.sin(vector_heading)

                new_target_point = Point(x = robot_position.x + new_x_dist, y = robot_position.y + new_y_dist, z = target_point.z)
                target_grid_position = PointToGridCoords(new_target_point)
                while processed_grid.data[target_grid_position[0], target_grid_position[1]] != 0 and follow_dist < (x_dist) ** 2 + y_dist ** 2:
                    new_x_dist = x_dist - follow_dist * math.cos(vector_heading)
                    new_y_dist = y_dist - follow_dist * math.sin(vector_heading)
            ...
            if found_point == False:
                new_target_point = robot_position

            past_follow_points.poses.append(Pose(position = target_point))
            if(len(past_follow_points.poses) > 100):
                past_follow_points.poses = past_follow_points.poses[-100:]
        ...
        print(len(past_follow_points.poses))
